...
const eventTypes: EventTypeDescription[] = [
...
const DraggableButton = ({ type, t, color }: DraggableButtonProps): JSX.Element => {
...
function getCurrentScrollTime(el: HTMLElement): string | null {
    ...
    for (let i = 0; i < slots.length; i++) {
        ...
        if (slotRect.top > rect.top + 100) {
            return (slots[i] as HTMLElement).dataset.time || null;
        }
    }
...
/** Shape of the argument FullCalendar passes to a custom time formatter (not exported by the package) */
interface VerboseFormattingArgLike {
    date: { hour: number; minute: number; second: number };
}

function formatter(date: VerboseFormattingArgLike): string {
    if (date.date.second) {
...
function Calendar(props: CalendarProps): JSX.Element | null {
    const [eventDialog, setEventDialog] = useState<string | null>(null);
    const [step, setStep] = useState<number>(
        props.dayStep || parseInt(window.localStorage.getItem('calendarStep') as string, 10) || 30,
    );
    const storageName = props.storageName || 'calendar';
    const ref = useRef<FullCalendar>(null);
    const dblClick = useRef<number | null>(null);
    ...
    const [calendarInterval, setCalendarInterval] = useState<{ start: Date | null; end: Date | null }>({
        start: null,
        end: null,
    });
    ...
    const events = useMemo(() => {
        ...
        props.events.forEach(event => {
            ...
            let name: string = event.common.name;
            if (event.native.oid) {
                let startValue: any = event.native.startValue;
                ...
                let endValue: any = event.native.intervals?.[0]?.value;
            ...
            if (event.native.cron) {
                ...
                if (Array.isArray(cronObject.months)) {
                    const rule = new RRule({
                        ...
                        bymonthday: Array.isArray(cronObject.dates) ? cronObject.dates : undefined,
                        // tzid: TIME_ZONE,
                    });

                    rule.between(calendarInterval.start || new Date(), calendarInterval.end || new Date()).forEach(
                        rruleTime => {
                ...
                const timeStart: Date | string = event.native.astro
                    ? SunCalc.getTimes(new Date(), props.systemConfig.latitude!, props.systemConfig.longitude!)[
                          event.native.astro
                      ]
                    : event.native.start!;

                if (event.native.astro && event.native.offset) {
                    (timeStart as Date).setMinutes((timeStart as Date).getMinutes() + event.native.offset);
                }
                ...
                _events.push({
                    ...
                    backgroundColor,
                    textColor,
                    start: serverDateToClient(timeStart, 'date', props.serverTimeZone) || undefined,
                    end:
                        serverDateToClient(
                            new Date(new Date(timeStart).getTime() + initialDuration),
                            'date',
    ...
    useEffect(() => {
        ...
        setTimeout(() => {
            ...
            ref.current
                ?.getApi()
                .scrollToTime(
                    `${now.getHours().toString().padStart(2, '0')}:${(step > now.getMinutes() ? 0 : now.getMinutes() - step).toString().padStart(2, '0')}:00`,
                );
        }, 300);
    ...
    useEffect(() => {
        if (props.isSimulation) {
            const calendar = ref.current?.getApi();
            calendar?.changeView(
                props.simulation?.native.interval === 'day' ? 'timeGridDay' : 'timeGridWeek',
                new Date(),
            );
        }
...
`}
            ...
            <div style={styles.container}>
                {!props.hideLeftBlock && !props.readOnly && (
                    <div style={styles.leftBlock}>
                        <Paper
                            elevation={4}
                            style={styles.leftPaper}
                        >
                            <div style={styles.leftContent}>
                                ...
                                {props.hideLeftBlockHint ? null : <hr style={styles.hr} />}
                                {props.hideLeftBlockHint ? null : (
                                    <div>{props.t('Use double click on calendar to add new events.')}</div>
                                )}
                                {props.hideLeftBlockHint ? null : <hr style={styles.hr} />}
                                {props.hideLeftBlockHint ||
                                currentView === 'dayGridMonth' ||
                                currentView === 'listMonth' ? null : (
                                    <FormControl
                                        fullWidth
                                        variant="standard"
                                    >
                                        ...
                                        <Select
                                            value={step}
                                            onChange={e => {
                                                const el = (ref.current as any)?.elRef?.current as
                                                    HTMLElement | undefined;
                                                if (el) {
                                                    const curTime = getCurrentScrollTime(el);
                                                    if (curTime) {
                                                        setTimeout(
                                                            () => ref.current?.getApi().scrollToTime(curTime),
                                                            300,
                                                        );
                                                    }
                                                }
                        ...
                        <FullCalendar
                            ...
                            headerToolbar={
                                props.hideTopBlock
                                    ...
                                    : {
                                          left:
                                              props.hideTopBlockButtons || props.isSimulation ? '' : 'prev,next today',
                                          center: props.isSimulation ? '' : 'title',
                                          right:
                                              props.hideTopBlockButtons || props.isSimulation
                                                  ? ''
                            ...
                            datesSet={(date: DatesSetArg) => {
                                if (
                                    date.start.toString() !== calendarInterval.start?.toString() ||
                                    date.end.toString() !== calendarInterval.end?.toString()
                                ) {
                                    lastEventTime.current = null;
                                    setCalendarInterval({
                                        start: date.start,
                            ...
                            eventResize={(event: EventResizeDoneArg) => {
                                lastEventTime.current = new Date(event.event.start!).getTime();
                                const eventData = props.events.find(
                                    _event => _event._id === event.event.extendedProps.eventId,
                                );
                                if (eventData?.native.intervals?.[0].timeOffset) {
                                    const newEvent: CalendarEvent = JSON.parse(JSON.stringify(eventData));
                                    newEvent.native.intervals![0].timeOffset! += event.endDelta.milliseconds;
                                    void props.setEvent(newEvent._id, newEvent);
                                    void props.updateEvents();
                                } else {
                                    event.revert();
                                }
                            }}
                            ...
                            //    console.log(event);
                            // }}
                            eventDrop={async (event: EventDropArg) => {
                                lastEventTime.current = new Date(event.event.start!).getTime();
                                const eventData = props.events.find(
                                    _event => _event._id === event.event.extendedProps.eventId,
                                );
                            ...
                            eventReceive={async (event: EventReceiveArg) => {
                                const newEvent: CalendarEvent = {
                                    ...
                                    native: {
                                        ...
                                        type: event.event.extendedProps.type as EventType,
                                        oid: '',
                                        startValue: '',
                                    },
                                    type: 'schedule',
                                ...
                                if (props.isSimulation) {
                                    ...
                                    cron.dows =
                                        props.simulation!.native.interval === 'day'
                                            ? [0, 1, 2, 3, 4, 5, 6]
                            ...
                            eventDidMount={(info: EventMountArg) => {
                                let icon: string | undefined = info.event.extendedProps.icon;
                                if (icon) {
                                    ...
                                    if (cnt.length) {
                                        ...
                                        if (title.length) {
                                            (title[0] as HTMLElement).style.marginLeft = '23px';
                                        }
                                    }
                                }
                            }}
                            select={async (selectInfo: DateSelectArg) => {
                                ...
                                if (
                                    (!dblClick.current || Date.now() - dblClick.current > 500) &&
                                    diff === 30 * 60 * 1000
                                ) {
                                ...
                                const newEvent: CalendarEvent = {
                                    ...
                                    native: {
                                        id: Date.now(),
                                        start: clientDateToServer(selectInfo.start, 'date', props.serverTimeZone),
                                        intervals:
                                            selectInfo.end && !props.isSimulation
                                                ? [
                                                      {
                                                          value: '',
                                                          timeOffset:
