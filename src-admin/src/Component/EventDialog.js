...
import { ColorPicker, SelectID, Confirm, Icon, Utils } from '@iobroker/adapter-react-v5';
    ...
    useEffect(() => {
        ...
        if (event.native?.oid) {
            if (!object || object._id !== event.native?.oid) {
                try {
                    // read object
                    props.socket.getObject(event.native?.oid).then(obj => {
                        ...
                        if (object || props.event.native.oid !== event.native.oid) {
                            getImage(obj, props.socket).then(newImage =>
                                changeEvent(newEvent => (newEvent.common.icon = newImage)),
                            );
                        }
        ...
        if (object.common.states) {
            return (
                ...
                >
                    ...
                    >
                        {Object.keys(object.common.states).map(option => (
                            <MenuItem
                                key={option}
                                value={option}
                            >
                                {object.common.states[option]}
                            </MenuItem>
                        ))}
                    </Select>
                </FormControl>
